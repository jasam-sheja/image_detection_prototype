...
public class Settings {
	    ...
	    while (eventType != XmlPullParser.END_DOCUMENT) {
	    	...
	    	case XmlPullParser.START_TAG:
	    		...
	    		if(name.equalsIgnoreCase(BlurTag)){
	    			...
	    			if(blurType == null)throw new XmlPullParserException("missing blur type attriput",parser,null);
	    			if(!validBlurType(blurType)) throw new XmlPullParserException("wrong blur type "+blurType,parser,null);
	    			blurSize = Integer.parseInt(parser.nextText());
	    			if(!validBlurSize(blurSize)) throw new XmlPullParserException("invalid blur size "+blurSize,parser,null);
	    		}else if(name.equalsIgnoreCase(Detector_Tag)){
	    			Detector = parser.nextText();
	    			if(!validDetector(Detector))  throw new XmlPullParserException("wrong Detector type "+Detector,parser,null);
	    		}else if(name.equalsIgnoreCase(Extractor_Tag)){
	    			Extractor = parser.nextText();
	    			if(!validExtractor(Extractor))  throw new XmlPullParserException("wrong Extractor type "+Extractor,parser,null);
	    		}else if(name.equalsIgnoreCase(Matcher_Tag)){
	    			String mds = parser.getAttributeValue(null, MatcherATTmin_dist);
	    			if(mds == null)throw new XmlPullParserException("missing blur type attriput",parser,null);
	    			minDist = Double.parseDouble(mds);	    			
	    			if(!validMinDist(minDist)) throw new XmlPullParserException("wrong minDist value "+minDist,parser,null);
	    			Matcher = parser.nextText();
	    			if(!validMatcher(Matcher)) throw new XmlPullParserException("wrong Matcher type "+Matcher,parser,null);
	    		}
	    		else{
	    			skip(parser);
	    		}
	...
	private static boolean validBlurType(String t){
		return (t.equalsIgnoreCase(BLUR_GAUSSIAN) ||
				...
				t.equalsIgnoreCase(BLUR_NONE) ||
				t.equalsIgnoreCase(BLUR_MEDIAN));	
	}
	...
	public void setBlurSize(int s){
		if(validBlurSize(s)&&isBlurActive()){
			blurSize = s;
			if((blurType.equalsIgnoreCase(BLUR_GAUSSIAN) || blurType.equalsIgnoreCase(BLUR_MEDIAN)) &&
					blurSize%2==0){
				--blurSize;
	...
	private static boolean validMatcher(String m){
		return (m.equalsIgnoreCase(Matcher_BRUTEFORCE)||
				m.equalsIgnoreCase(Matcher_BRUTEFORCE_HAMMING));
	}
	...
	private String filepath = Environment.getDataDirectory().toString() + "/" + "jsh.xml";
	...
	public static Settings getInstance(){
		if(appsettings == null)
			appsettings = new Settings(Environment.getExternalStorageDirectory().toString() + "/" + "settings.xml");
		return appsettings;
	}
