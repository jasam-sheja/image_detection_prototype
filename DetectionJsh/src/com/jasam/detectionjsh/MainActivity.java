...
public class MainActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		String storedUsername = sessionManager.getUserDetails().get(SessionManager.KEY_USERNAME);
		String storedPassword = sessionManager.getUserDetails().get(SessionManager.KEY_PASSWORD);
		...
		// If was logged in and authorizator created a token (token and secret provided)
		// And open new intent
		if (sessionManager.isLoggedIn() && authorizator.getToken() != null) {
			Intent intent = new Intent(getApplicationContext(), UserTakeActivity.class);
			...
			((Button) findViewById(R.id.btnLogin)).setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if (auth != null) {
						// Store username, password and token credintials to remember
						if (rememberMe()) {
							sessionManager.createLoginSession(username, password,auth.getToken(),auth.getSecret());
						}
