...
public class UserTakeActivity4 extends Activity implements CvCameraViewListener2,OnTouchListener{
	private static final String  TAG = "UserTake::Activity-4";
	
	private CameraBridgeVeiwCustom mOpenCvCameraView;
	private ImageDetectionFilterTemp filter;
        ...
        public void onManagerConnected(int status) {
            switch (status) {
                case LoaderCallbackInterface.SUCCESS:
                {
                    ...
                    filter = new ImageDetectionFilterTemp();
                    for(String path:getIntent().getStringArrayListExtra("images")){
                    	filter.addReference(path);
                    }
                    
                    /* Now enable camera view to start receiving frames */
                    mOpenCvCameraView.setOnTouchListener(UserTakeActivity4.this);
