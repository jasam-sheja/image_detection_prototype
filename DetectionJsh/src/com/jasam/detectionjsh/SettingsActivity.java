...
public class SettingsActivity extends ListActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		String[] objs = new String[] { "blur", "blur size", "detector",
				"extractor", "matcher", "Min Dist Matching", "Save" };
	...
	private static class mylistadapter extends ArrayAdapter<String> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			LinearLayout ll = (LinearLayout) row
					.findViewById(R.id.settings_row_linearlayout);
			TextView tv = (TextView) row
					.findViewById(R.id.settings_row2_textView);
			...
			if (getItem(position).equalsIgnoreCase("blur")) {
				...
				sp.setAdapter(new ArrayAdapter<String>(getContext(),
						...
						Settings.BLUR_TYPES));
				sp.setSelection(Settings.BLUR_TYPES.indexOf(settings
						.getBlurType()));
				...
				sp.setAdapter(new ArrayAdapter<String>(getContext(),
						android.R.layout.simple_spinner_item,
						Settings.Detector_TYPES));
				sp.setSelection(Settings.Detector_TYPES.indexOf(settings
						.getDetectorName()));
				sp.setOnItemSelectedListener(new OnItemSelectedListener() {
					...
					public void onItemSelected(AdapterView<?> parent,
							View view, int position, long id) {
						settings.setDetectorName(Settings.Detector_TYPES
								.get(position));
					}
				...
				sp.setAdapter(new ArrayAdapter<String>(getContext(),
						android.R.layout.simple_spinner_item,
						Settings.Extractor_TYPES));
				sp.setSelection(Settings.Extractor_TYPES.indexOf(settings
						.getExtractorName()));
				sp.setOnItemSelectedListener(new OnItemSelectedListener() {
					...
					public void onItemSelected(AdapterView<?> parent,
							View view, int position, long id) {
						settings.setExtractorName(Settings.Extractor_TYPES
								.get(position));
					}
				...
				sp.setAdapter(new ArrayAdapter<String>(getContext(),
						android.R.layout.simple_spinner_item,
						Settings.Matcher_TYPES));
				sp.setSelection(Settings.Matcher_TYPES.indexOf(settings
						.getMatcherName()));
				sp.setOnItemSelectedListener(new OnItemSelectedListener() {
					...
					public void onItemSelected(AdapterView<?> parent,
							View view, int position, long id) {
						settings.setMatcherName(Settings.Matcher_TYPES
								.get(position));
					}
	...
	public static class myItemClickListner implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,
				long id) {
			Log.i(TAG, "saving settings to: "
					+ Settings.getInstance().getFilePath());
			if (((String) parent.getItemAtPosition(position))
					.equalsIgnoreCase("save")) {
				try {
					Settings.getInstance().writeSettings();
				} catch (IOException e) {
					Toast.makeText(parent.getContext(), "fail to save",
