	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		go.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				path = (sdcard.isChecked() ? Environment.getRootDirectory()
						.getAbsolutePath() : Environment
				...
				if (new File(path).exists()) {
					Intent intent = new Intent(MemoryPhotoActivity.this,
							MemoryPhotoSubActivity.class);
					intent.putExtra("path", path);
					startActivity(intent);
				} else {
					Toast.makeText(MemoryPhotoActivity.this,
							"no such image :: " + path, Toast.LENGTH_SHORT)
							.show();
				}
			}
		});
