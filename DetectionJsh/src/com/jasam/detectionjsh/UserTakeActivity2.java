...
public class UserTakeActivity2 extends Activity implements
	...
	private void Blur(Mat gray) {
		if (settings.getBlurType().equalsIgnoreCase(Settings.BLUR_HOMOGENEOUS)) {
			...
			Imgproc.blur(
					mGray,
					mGray,
					new org.opencv.core.Size(settings.getBlurSize(), settings
							.getBlurSize()));
		} else if (settings.getBlurType().equalsIgnoreCase(
				Settings.BLUR_GAUSSIAN)) {
			Log.i(TAG, "blur:: using" + settings.getBlurType());
			Imgproc.GaussianBlur(mGray, mGray, new org.opencv.core.Size(
					settings.getBlurSize(), settings.getBlurSize()), 0, 0);
		} else if (settings.getBlurType()
				.equalsIgnoreCase(Settings.BLUR_MEDIAN)) {
			Log.i(TAG, "blur:: using" + settings.getBlurType());
			Imgproc.medianBlur(mGray, mGray, settings.getBlurSize());
		}
	}

	private List<DMatch> myMatcher(List<DMatch> matches,
			List<KeyPoint> keyPoints, List<KeyPoint> targetKeyPoints) {
		...
		for (int i = 0; i < matches.size(); ++i) {
			p[i] = keyPoints.get(matches.get(i).queryIdx).pt.y
					...
					* (keyPoints.get(matches.get(i).queryIdx).pt.y - targetKeyPoints
							.get(matches.get(i).trainIdx).pt.y)
					/ (keyPoints.get(matches.get(i).queryIdx).pt.x - targetKeyPoints
							.get(matches.get(i).trainIdx).pt.x);
			avg += p[i];
		}
		avg /= p.length;
		Log.i(TAG, "avg p = " + avg);
	...
	public Mat onCameraFrame(CvCameraViewFrame inputFrame) {
		...
		if (viewmatches) {
			...
			try {
				mDescriptors = new Mat(targetdescriptors.rows(),
						targetdescriptors.cols(), targetdescriptors.type());
				...
				List<KeyPoint> lKeyPoints = mKeypoints.toList(), lTargetKeyPoints = targetKeypoint
						.toList();
				List<DMatch> lGoodDMatches = new ArrayList<DMatch>();

				if (lmatches.size() > 3
						&& lmatches.get(3).distance < settings.getMinDist()) {
					List<DMatch> good_dmatches_temp = new ArrayList<DMatch>();
					good_dmatches_temp = lmatches.subList(0,
							Math.max(4, lmatches.size() / 10));

					if (good_dmatches_temp.get(good_dmatches_temp.size() - 1).distance < settings
							.getMinDist()) {
						lGoodDMatches = good_dmatches_temp;
					} else {
						lGoodDMatches = myMatcher(good_dmatches_temp,
								lKeyPoints, lTargetKeyPoints);
					}
				}
				...
				for (int i = 0; i < lGoodDMatches.size(); ++i) {
					Core.circle(img_keypoint, lKeyPoints.get(lGoodDMatches
							.get(i).queryIdx).pt,
							(int) mGray.size().width / 200, new Scalar(25, 200,
									255), -1);
					Core.circle(img_keypoint, lTargetKeyPoints
							.get(lGoodDMatches.get(i).trainIdx).pt, (int) mGray
							.size().width / 200, new Scalar(255, 25, 200), 1);
					Core.line(
							img_keypoint,
							lKeyPoints.get(lGoodDMatches.get(i).queryIdx).pt,
							lTargetKeyPoints.get(lGoodDMatches.get(i).trainIdx).pt,
							new Scalar(200, 255, 25), 1);
				}
			} catch (Exception e) {
				e.printStackTrace();
				...
				Log.d(TAG,
						"mGray size : " + mGray.rows() + " | " + mGray.cols());
				Log.d(TAG, "target size : " + targetmGray.rows() + " | "
						+ targetmGray.cols());

				Log.d(TAG, "mGray descriptors type : " + mDescriptors.type());
				Log.d(TAG, "target descriptors : " + targetdescriptors.type());

				Log.d(TAG, "mGray descriptors size : " + mDescriptors.rows()
						+ " | " + mDescriptors.cols());
				Log.d(TAG,
						"target descriptors size : " + targetdescriptors.rows()
								+ " | " + targetdescriptors.cols());
				viewmatches = false;
				return mGray;
			}
		} else {
			...
			for (int i = 0; i < keyPoints.size(); ++i) {
				Core.circle(img_keypoint, keyPoints.get(i).pt,
						(int) mGray.size().width / 150,
						new Scalar(255, 25, 200), 2);
			}
		}
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		if (viewmatches) {
			try {
				...
				extractor.compute(targetmGray, targetKeypoint,
						targetdescriptors);
			...
			Toast.makeText(this, " target frame saved", Toast.LENGTH_SHORT)
					.show();
		} else {
